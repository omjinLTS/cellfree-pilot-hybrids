...
\begin{frame}{Cell-Free Massive MIMO}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.52\textwidth}
            ...
            \vspace{0.2em}
            \smallnote{References: Gao \emph{et al.} (IEEE TVT 2024); Mussbah \emph{et al.} (IEEE CL 2024).}
        \end{column}
        \begin{column}{0.44\textwidth}
            \centering
            \begin{tikzpicture}[
                    ap/.style={draw, circle, fill=blue!20, minimum size=0.42cm, inner sep=0pt, font=\tiny},
                    ue/.style={draw, circle, fill=orange!30, minimum size=0.42cm, inner sep=0pt, font=\tiny},
                    cpu/.style={draw, rounded corners, fill=gray!15, minimum width=1.5cm, minimum height=0.5cm, font=\scriptsize},
                    bh/.style={dashed, draw=gray!55, thin}
                ]
                \node[cpu] (cpu) at (0, 2.1) {CPU};
                \node[ap] (a1) at (-2.0, 0.7) {AP};
                \node[ap] (a2) at (-0.7, 1.1) {AP};
                \node[ap] (a3) at (0.7, 1.1) {AP};
                \node[ap] (a4) at (2.0, 0.7) {AP};
                \node[ap] (a5) at (-1.3, -0.7) {AP};
                \node[ap] (a6) at (1.3, -0.7) {AP};
                \node[ue] (u1) at (-0.7, 0.05) {UE};
                \node[ue] (u2) at (0.7, 0.05) {UE};
                \node[ue] (u3) at (0, -1.1) {UE};
                \foreach \a in {a1,a2,a3,a4,a5,a6} \draw[bh] (\a) -- (cpu);
                \draw[->, thick, blue!65] (a2) -- (u1);
                \draw[->, thick, blue!65] (a5) -- (u1);
                \draw[->, thick, blue!65] (a3) -- (u2);
                \draw[->, thick, blue!65] (a4) -- (u2);
                \draw[->, thick, blue!65] (a5) -- (u3);
                \draw[->, thick, blue!65] (a6) -- (u3);
            \end{tikzpicture}

            {\scriptsize Distributed APs \(\to\) CPU; multiple APs cooperatively serve each UE.}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Two Mitigation Angles, One Combined Direction}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.48\textwidth}
            ...
            \vspace{0.3em}
            \smallnote{Limitation: AP-level matching does not resolve beam direction within a shared AP.}
        \end{column}
        \begin{column}{0.48\textwidth}
            ...
            \vspace{0.3em}
            \smallnote{Limitation: dense beam-overlap graphs inflate the chromatic number and pilot overhead.}
        \end{column}
    \end{columns}
...
\begin{frame}{Mussbah Beam-Domain Method}
    ...
    \vspace{0.4em}
    \smallnote{Beam-domain conflicts capture finer interference than AP-domain. Trade-off: dense graphs raise the chromatic number and inflate pilot overhead.}
\end{frame}
...
\begin{frame}{Gao AP-Domain Matching}
    ...
    \vspace{0.3em}
    \smallnote{Limitation: AP-level matching cannot distinguish two UEs that share an AP through different beams.}
\end{frame}
...
\begin{frame}{BRM: Matching Variables and Constraints}
    ...
    \vspace{0.35em}
    \smallnote{Matching follows a deferred-acceptance protocol: UEs propose to their strongest resources, and each resource accepts up to \(Q_{\mathrm{res}}\) UEs ranked by beam power.}
\end{frame}
