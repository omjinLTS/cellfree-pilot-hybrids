...
\begin{frame}{Weighted Assignment Chain}
    \small
    \begin{center}
        \begin{tikzpicture}[
                node distance=0.55cm,
                box/.style={draw, rounded corners, align=center, fill=purple!8, font=\scriptsize, minimum width=2.5cm, minimum height=0.60cm},
                arr/.style={->, thick, >=Stealth}
            ]
            \node[box] (w) {\(\waa,\wai,\wia\)};
            \node[box, right=0.5cm of w] (W) {\(W_{u,v}\)};
            \node[box, right=0.5cm of W] (M) {\(M_{u,v}^{B}\)};
            \node[box, below=0.65cm of M] (S) {\(S_r,\ C(u,p)\)};
            \node[box, left=0.5cm of S] (p) {\(p_u^{\star}\)};
            \draw[arr] (w) -- (W);
            \draw[arr] (W) -- (M);
            \draw[arr] (M) -- (S);
            \draw[arr] (S) -- (p);
    ...
    \begin{block}{Assignment implication}
        A single weighted signal \((\waa,\wai,\wia)\) drives both group priority and pilot reuse cost.
    \end{block}
\end{frame}
...
\begin{frame}{Main Result: Moderate User Load}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.58\textwidth}
            \centering
            \includegraphics[width=\linewidth,height=0.68\textheight,keepaspectratio]{presentation_6method/presentation_clean_load_crossover_se.png}
        \end{column}
        \begin{column}{0.38\textwidth}
            \footnotesize
            \textbf{Evaluation configuration}
            \begin{tightitemize}
                \item \(L=200,\ N=8,\ K=25{-}50\)
                \item \(\tauc=150,\ \taudesign=15\)
                \item 200 setups
            \end{tightitemize}

            \vspace{0.25em}
            \textbf{Observed performance at \(K=50\)}
            \begin{tightitemize}
                \item \BRM{}: \(+6.66\%\) mean SE.
                \item \TopN{}: \(+5.35\%\) mean SE.
                \item Gao matches Random within noise.
                \item Mussbah loses from inflated \(\taup\).
            \end{tightitemize}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Actual Pilot Count Explains the Trend}
    \small
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.58\textwidth}
            \centering
            \includegraphics[width=\linewidth,height=0.68\textheight,keepaspectratio]{presentation_6method/presentation_clean_pilot_count_vs_k.png}
        \end{column}
        \begin{column}{0.38\textwidth}
            \textbf{Pilot-overhead profile}
            \begin{tightitemize}
                \item Random and Gao consume the full design budget.
                \item Mussbah's chromatic number grows steeply with load.
                \item \BRM{} holds \(\taup\) below the design budget across all load levels.
            \end{tightitemize}

            \vspace{0.45em}
            \textbf{Observed pilot count at \(K=50\)}
            \begin{tightitemize}
                \item Random / Gao: \(\taup=15\).
                \item \TopN{}: \(\taup\approx7.79\).
                \item \BRM{}: \(\taup\approx5.29\).
                \item Mussbah: \(\taup\approx45.76\).
            \end{tightitemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Antenna Sweep: Robustness Across \(N\)}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.49\textwidth}
            \centering
            \includegraphics[width=\linewidth,height=0.55\textheight,keepaspectratio]{presentation_n_sweep_6method/n_sweep_avg_se_vs_n.png}
        \end{column}
        \begin{column}{0.49\textwidth}
            \centering
            \includegraphics[width=\linewidth,height=0.55\textheight,keepaspectratio]{presentation_n_sweep_6method/n_sweep_avg_ee_vs_n.png}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.47\textwidth}
            \scriptsize
            \textbf{Evaluation configuration:} \(L=200\), \(K=50\), \(N=1,\ldots,8\), \(\tauc=150\), \(\taudesign=15\), 200 setups.
        \end{column}
        \begin{column}{0.49\textwidth}
            \scriptsize
            \textbf{Observed trend}
            \begin{tightitemize}
                \item \BRM{} attains the highest mean SE across all tested \(N\).
                \item At \(N=8\): \(+6.57\%\) mean SE, \(+12.01\%\) EE proxy.
                \item \TopN{} provides a simpler AP-domain alternative with consistent positive gain.
            \end{tightitemize}
        \end{column}
    \end{columns}
\end{frame}
